...
// SMVDU-TITAN-X SoC — Gate-Level Simulation (GLS) Testbench
`timescale 1ns/1ps
module tb_titan_x_final;
    ...
    // ── DUT (Gate-Level Netlist Instance with DFT ports connected) ───────────
    titan_x_top dut (
        .scan_in        (1'b0),
        .scan_enable    (1'b0),
        .scan_out       (),
    ...
    // ── Waveform Dump ────────────────────────────────────────────────────────
    initial begin
        $dumpfile("titan_x_soc/05_GLS/Output_Files/gls_waveform.vcd");
        $dumpvars(0, tb_titan_x_final);
    end
    ...
    initial begin
        $display("===========================================");
        $display(" SMVDU-TITAN-X Gate-Level Simulation (GLS) ");
        $display("===========================================");
        ...
        sys_rst_n = 1'b1;
        $display("[%0t] Reset released. Gate-level netlist booting...", $time);
        ...
        $display("[%0t] GLS HART(s) active: 0b%b", $time, harts_active);
        ...
        if (harts_active == 5'b11111)
            $display("[PASS] Gate-level simulation: All 5 harts active after reset.");
        else
            $display("[WARN] GLS harts_active = %b (expected 5'b11111)", harts_active);

        // Check DDR clock output
        if (ddr_ck_p !== ddr_ck_n)
            $display("[PASS] GLS DDR differential clock running.");
        else
            $display("[WARN] GLS DDR differential clock not oscillating.");

        $display("[%0t] GLS completed (%0d cycles).", $time, cycle_cnt);
        $display("===========================================");
        $finish;
    end
    ...
    initial begin
        ...
        $display("[TIMEOUT] GLS exceeded 5ms — possible hang.");
        $finish;
    end
endmodule
