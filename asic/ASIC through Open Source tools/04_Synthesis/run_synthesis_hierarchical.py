...
submodules = [
    ...
    # --- LEVEL 2: MID-LEVEL BLOCKS ---
    {
        "name": "rv_core_top",
        "sources": [
            f"{RTL_DIR}/cpu_complex/rv_core/rv_fetch.v",
            f"{RTL_DIR}/cpu_complex/rv_core/rv_decode.v",
            f"{RTL_DIR}/cpu_complex/rv_core/rv_execute.v",
            f"{RTL_DIR}/cpu_complex/rv_core/rv_mem.v",
            f"{RTL_DIR}/cpu_complex/rv_core/rv_writeback.v",
            f"{RTL_DIR}/cpu_complex/rv_core/rv_core_top.v"
        ],
        "dependencies": []
    },
    {
        "name": "l2_cache_top",
        "sources": [
            f"{RTL_DIR}/memory_subsystem/l2_tag_array.v",
            f"{RTL_DIR}/memory_subsystem/l2_data_array.v",
            f"{RTL_DIR}/memory_subsystem/l2_cache_ctrl.v",
            f"{RTL_DIR}/memory_subsystem/l2_cache_top.v"
        ],
        "dependencies": [],
        "use_sram_lib": True
    },
    ...
    # --- LEVEL 3: COMPLEXES ---
    {
        ...
        "sources": [
            ...
            f"{RTL_DIR}/cpu_complex/rv_core/rv_writeback.v",
            f"{RTL_DIR}/cpu_complex/rv_core/rv_core_top.v",
            f"{RTL_DIR}/cpu_complex/clint.v",
            f"{RTL_DIR}/cpu_complex/plic.v",
            f"{RTL_DIR}/cpu_complex/cpu_complex_top.v"
        ],
        "dependencies": []
    }
]
...
def synthesize_top():
    ...
    with open(tcl_script_path, "w") as f:
        ...
        for module in submodules:
            if module["name"] in ["reset_sync", "axi4_crossbar", "axi4_to_ahb"]:
                continue
            dep_netlist = f"{SUBMODULES_DIR}/{module['name']}_synth.v"
            f.write(f"read_verilog -lib {dep_netlist}\n")
            
        # 3. Read top-level wrapper source and parameterized child RTLs
        f.write(f"read_verilog {RTL_DIR}/common/reset_sync.v\n")
        f.write(f"read_verilog {RTL_DIR}/interconnect/axi4_crossbar.v\n")
        f.write(f"read_verilog {RTL_DIR}/interconnect/axi4_to_ahb.v\n")
    ...
    # Clean up temp TCL script only on success
    if res.returncode == 0 and os.path.exists(tcl_script_path):
        os.remove(tcl_script_path)
